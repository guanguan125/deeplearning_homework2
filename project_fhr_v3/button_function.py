...
from tracking.track import run__
class App:
    ...
    def go_to_other_page(self):
        ...
        entry.pack()
        address_button = tk.Button(self.root, text="提交", command=lambda: self.run_model(entry=entry))
        address_button.pack(pady=10)
        

    def run_model(self,entry):
        user_input = entry.get()  # 获取输入框的内容
        print("输入的字符串是：", user_input)
        run__(user_input)
        finish_label = tk.Label(self.root, text="完成检测！", font=("Arial", 14))
